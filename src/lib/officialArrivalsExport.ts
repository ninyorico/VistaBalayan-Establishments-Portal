...
const normalizeOvernightThirdMetric = (sheet: ExcelJS.Worksheet, startRow: number, endRow: number) => {
  ...
  titleCell.style = JSON.parse(JSON.stringify(sheet.getCell("I42").style));
...
const formatGrandTotalSheet = (sheet: ExcelJS.Worksheet, establishmentEndRow: number, totalRow: number) => {
  ...
  const establishmentNameStyle = JSON.parse(JSON.stringify(establishmentNameTemplate.style));
